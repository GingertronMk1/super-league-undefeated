...
export default function useGame() {
  ...
  const statModifiers: Ref<typeof INITIAL_STAT_MODIFIERS> =
    ...
    return teams.map((team1: TableTeam) =>
      teams
        .filter((team2: TableTeam) => JSON.stringify(team1) !== JSON.stringify(team2))
        .map((team2): Match => simulateGame(team1, team2))
      ).flat()
    }

  function simulatePlayoff(team1: TableTeam, team2: TableTeam): [Match, TableTeam] {
    const match = simulateGame(team1, team2, false)
      return [
        match,
        match.result === team1.name ? team1 : team2
      ]
    }
  function simulatePlayoffs(
    ...
    const [eliminator1, eliminator1Winner] = simulatePlayoff(third, sixth)
    const [eliminator2, eliminator2Winner] = simulatePlayoff(fourth, fifth)
    ...
    if (eliminator1Winner === third && eliminator2Winner === fourth) {
      [ semiFinal1, semiFinal1Winner ] = simulatePlayoff(second, third);
      [ semiFinal2, semiFinal2Winner] = simulatePlayoff(first, fourth);
    } else if (eliminator1Winner === third && eliminator2Winner === fifth) {
      [semiFinal1, semiFinal1Winner] = simulatePlayoff(second, third);
      [semiFinal2, semiFinal2Winner] = simulatePlayoff(first, fifth);
    } else if (eliminator1Winner === sixth && eliminator2Winner === fourth) {
      [semiFinal1, semiFinal1Winner] = simulatePlayoff(second, fourth);
      [semiFinal2, semiFinal2Winner] = simulatePlayoff(first, sixth);
    } else {
      [semiFinal1, semiFinal1Winner] = simulatePlayoff(second, fifth);
      [semiFinal2, semiFinal2Winner] = simulatePlayoff(first, sixth);
    }
    const [grandFinal, grandFinalWinner]: [Match, TableTeam] = simulatePlayoff(first, semiFinal1Winner)
    return {
      eliminator1,
      eliminator1Winner,
      eliminator2,
